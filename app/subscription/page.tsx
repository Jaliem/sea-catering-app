...
export default function SubscriptionPage() {
	...
	useEffect(() => {
		if (formData.plan && formData.mealTypes.length > 0 && formData.deliveryDays.length > 0) {
			const selectedPlanData = mealPlans.find((plan) => plan.id === formData.plan)
			if (selectedPlanData) {
			...
			try {
				const response = await fetch("/api/subscription", {
					...
					body: JSON.stringify({
						...
						// Add more fields as needed to match your Prisma model
					}),
				})
				if (!response.ok) throw new Error("Failed to submit subscription")
											...
											>
												<div className="flex justify-between">
													...
													<span className="font-medium">
														{mealPlans.find((p) => p.id === formData.plan)?.name}
													</span>
												</div>
												<div className="flex justify-between">
											...
											>
												...
												<div className="space-y-2">
													...
													>
														Calculation:{" "}
														{formatPrice(mealPlans.find((p) => p.id === formData.plan)?.price || 0)} ×{" "}
														{formData.mealTypes.length} × {formData.deliveryDays.length} × 4.3
													</motion.div>
													<motion.div
